...
col2.metric("最大搜索窗口", str(st.session_state.recent_match_limit))
...
2. 识别逻辑：自动选择 recent matches 总量最少的玩家作为锚点，并在最大搜索窗口内校验其他玩家是否共同出现。
3. 命中规则：候选对局只保留至少 {MIN_HIT_PLAYER_COUNT} 名输入玩家共同出现的对局。
4. 分类信息：`gameMode` 只用于展示房间类别和队伍模式。
5. 参赛者名单：可基于 `roster` 生成主名单，并可选导入报名表补 QQ。
6. 系统设置：当前只保留 PUBG API Key、平台和最大搜索窗口。
"""
)
